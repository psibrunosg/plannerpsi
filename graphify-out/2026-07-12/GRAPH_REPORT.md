...
## Corpus Check
- 109 files · ~63,957 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 604 nodes · 624 edges · 82 communities (51 shown, 31 thin omitted)
...
### Community 44 - "MindMapBoard.tsx"
Cohesion: 0.67
Nodes (3): heatColor(), Stats(), WEEKDAY_LABELS

### Community 45 - "StudySidebar.tsx"
Cohesion: 0.50
Nodes (3): Toast, ToastState, useToastStore

## Knowledge Gaps
- **274 isolated node(s):** `$schema`, `temp.js`, `TaskForm_remote.tsx`, `typescript`, `oxc` (+269 more)
...
- **Why does `react` connect `devDependencies` to `ec`, `yc`, `react`, `constructor`, `compilerOptions`, `compilerOptions`, `motion.ts`, `manifest.json`, `drive.ts`, `.oxlintrc.json`, `sr`, `spotifyAuth.ts`, `studyStats.ts`, `ProcedureFlow.tsx`, `Tasks.tsx`, `mo`, `StudyTranscript.tsx`, `pushManager.ts`, `MindMapBoard.tsx`, `icsParser.ts`, `notificationManager.ts`, `mindMapStore.ts`, `spotifyStore.ts`, `taskStore.ts`, `index.ts`, `spotifyApi.ts`, `planningStore.ts`, `procedureStore.ts`, `Focus.tsx`, `Settings.tsx`?**
  _High betweenness centrality (0.164) - this node is a cross-community bridge._
- **Why does `TaskForm()` connect `uc` to `devDependencies`?**
