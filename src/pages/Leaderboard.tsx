...
function getProfileName(profile: Partial<Profile>) {
...
function getProfileInitial(profile: Partial<Profile>) {
    ...
    <div className="space-y-6">
      ...
      {loading && sortedProfiles.length === 0 ? (
        <div className="glass-card flex min-h-[280px] flex-col items-center justify-center p-8 text-center">
          ...
          <p className="text-sm text-text-muted">Carregando ranking da equipe...</p>
        </div>
      ) : sortedProfiles.length === 0 ? (
        <div className="glass-card flex min-h-[280px] flex-col items-center justify-center p-8 text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-accent/10 mb-4">
            <Trophy className="h-8 w-8 text-accent opacity-80" />
          </div>
          <h3 className="text-lg font-semibold text-text-primary mb-1">Ranking em Formação</h3>
          <p className="text-sm text-text-muted max-w-md">
            Nenhum perfil cadastrado na equipe ou sem pontuações ativas no momento. Realize tarefas e sessões de estudo para inaugurar o placar!
          </p>
        </div>
      ) : (
                ...
                {top3[1] && (
                  <motion.div variants={staggerItem} className="flex flex-col items-center">
                    <div className="w-16 h-16 rounded-full bg-surface-hover border-4 border-slate-300 flex items-center justify-center mb-4 z-10 shadow-lg shadow-slate-300/20">
                      <span className="text-xl font-bold text-slate-300">{getProfileInitial(top3[1])}</span>
                    </div>
                    <div className="text-center mb-2 z-10 max-w-[120px]">
                      <p className="font-semibold text-text-primary truncate" title={getProfileName(top3[1])}>{getProfileName(top3[1])}</p>
                ...
                {top3[0] && (
                  <motion.div variants={staggerItem} className="flex flex-col items-center">
                    ...
                    <div className="w-20 h-20 rounded-full bg-surface-hover border-4 border-yellow-400 flex items-center justify-center mb-4 z-10 shadow-xl shadow-yellow-400/20">
                      <span className="text-3xl font-bold text-yellow-400">{getProfileInitial(top3[0])}</span>
                    </div>
                    <div className="text-center mb-2 z-10 max-w-[140px]">
                      <p className="font-bold text-lg text-text-primary truncate" title={getProfileName(top3[0])}>{getProfileName(top3[0])}</p>
                ...
                {top3[2] && (
                  <motion.div variants={staggerItem} className="flex flex-col items-center">
                    <div className="w-16 h-16 rounded-full bg-surface-hover border-4 border-amber-600 flex items-center justify-center mb-4 z-10 shadow-lg shadow-amber-600/20">
                      <span className="text-xl font-bold text-amber-600">{getProfileInitial(top3[2])}</span>
                    </div>
                    <div className="text-center mb-2 z-10 max-w-[120px]">
                      <p className="font-semibold text-text-primary truncate" title={getProfileName(top3[2])}>{getProfileName(top3[2])}</p>
                ...
                {others.map((profile, index) => (
                  <div key={profile.id} className="p-4 flex items-center gap-4 hover:bg-surface-hover transition-colors">
                    ...
                    <div className="w-10 h-10 rounded-full bg-surface-hover flex items-center justify-center shrink-0">
                      <span className="text-sm font-medium text-text-secondary">{getProfileInitial(profile)}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-text-primary truncate" title={getProfileName(profile)}>{getProfileName(profile)}</p>
